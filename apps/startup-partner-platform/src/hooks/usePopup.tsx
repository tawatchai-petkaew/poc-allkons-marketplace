  ...
  switch (statusCode) {
    ...
    case 403:
      return {
        iconClass: 'ri-information-fill',
        message: 'การเข้าถึงถูกปฏิเสธ',
        description: 'คุณไม่ได้รับสิทธิ์ในการเข้าถึงหน้านี้ กรุณาติดต่อผู้ดูแลระบบ',
      };
    case 404:
      return {
...
const usePopup = () => {
  ...
  const PopupComponent = useCallback(() => {
    ...
    const displayDescription = description || (errorDetails ? errorDetails.description : '');
    ...
    return (
      ...
      >
        <div className="flex flex-col items-center text-center">
          <SectionIcon iconClass={iconConfig.iconClass} type={popupState.type} />
          <div className="mt-5">
            <Typography variant={isMobile ? `h1` : `h3`} className="!text-text-secondary">
              {displayTitle}
            </Typography>
          </div>
          {displayDescription && (
            <div className="mt-2">
              <Typography
                ...
                className="!text-text-quinary whitespace-pre-line"
              >
                {displayDescription}
              </Typography>
            </div>
