...
import { UserInviteStatus } from "@/constants/enum/invitation.enum";
...
export default function InviteMemberPopup({
  ...
  const orgDetail = organization?.organizationDetail?.organization;

  const [merchantRoles, setMerchantRoles] = useState<Record<number, number>>({});
  const [selectedMerchants, setSelectedMerchants] = useState<Set<number>>(new Set());
  const lastInitializedMerchants = useRef<string>('');
  ...
  const { data: merchantsData, isLoading: isLoadingMerchants } = useQuery({
    queryKey: ["merchantsByOrganization", organizationUuid],
    queryFn: () => getMerchantByOrganization(organizationUuid),
    enabled: isVisible && !!organizationUuid,
  });

  const merchants: IMerchantItem[] = useMemo(
    () => merchantsData?.data ?? [],
    [merchantsData?.data]
  );
  ...
  const { mutate: createInvite, isPending: isCreatingInvitation } = useMutation(
    {
      mutationFn: async (payload: {
        ...
        merchantInfo: Array<{ merchantId: number; roleId: number }>;
      }) => {
      ...
      onSuccess: () => {
        ...
        form.setFieldValue(
          "role",
          rolesQuery?.data?.roles?.find((role: IRole) => role.name === "SUPER_ADMIN")
            ?.id,
        );
  ...
  const handleClose = () => {
    ...
    // Reset merchant selections to defaults
    if (merchants.length > 0) {
      const defaultRoleId = rolesQuery?.data?.roles?.find((role: IRole) => role.name === "SUPER_ADMIN")?.id;
      if (defaultRoleId) {
        setSelectedMerchants(new Set(merchants.map((m) => m.id)));
        setMerchantRoles(Object.fromEntries(merchants.map((m) => [m.id, defaultRoleId])));
      }
    }
  ...
  }, [rolesQuery?.data?.roles, form]);

  useEffect(() => {
    // Initialize merchant state when popup opens and merchants change
    if (isVisible && merchants.length > 0 && merchantIds !== lastInitializedMerchants.current) {
      const defaultRoleId = rolesQuery?.data?.roles?.find((role: IRole) => role.name === "MEMBER")?.id;
      if (defaultRoleId) {
        lastInitializedMerchants.current = merchantIds;
    ...
    if (!isVisible) {
      lastInitializedMerchants.current = '';
    }
  }, [isVisible, merchantIds, rolesQuery?.data?.roles, merchants]);
  ...
  }, [errorRole, showPopup, refetchRoles]);
  ...
  const debouncedEmail = useDebounce(form.getFieldValue("email"), 300);

  useEffect(() => {
    if (debouncedEmail && validateEmail(debouncedEmail)) {
      validateEmailMutation(debouncedEmail);
    }
        ...
        <div>
          ...
          <div className="mt-6">
            <div className="flex items-center gap-2 mb-1">
              <Typography variant="paragraph-medium" className="!text-text-primary !font-medium">
                เพิ่มเป็นสมาชิกร้านค้า/สาขา
              </Typography>
              <BadgeLabel
                text="ระบุอย่างน้อย 1 ร้าน"
                color="neutral"
                variant="outlined"
                rounding="pill"
                size="small"
            ...
            {orgDetail && (
              <Typography variant="paragraph-small" className="!text-text-secondary pb-4">
                {orgDetail.juristicInfo?.prefix} {orgDetail.organizeName} {orgDetail.juristicInfo?.subfix}
              </Typography>
            )}
            <div className="flex flex-col">
              {/* ทุกร้านค้า/สาขา — select all row */}
              <div className="flex items-center gap-3 py-3 border-t border-border-primary">
                <Checkbox
                  checked={merchants.length > 0 && selectedMerchants.size === merchants.length}
                ...
                />
                <Typography variant="paragraph-small" className="!text-text-primary flex-1">
                  ทุกร้านค้า/สาขา
                </Typography>
              </div>
              {/* รายการร้านค้าแต่ละร้าน */}
              {isLoadingMerchants ? (
                ...
                merchants.map((merchant) => (
                  <div
                    key={merchant.id}
                    className="flex items-center gap-3 py-3 border-b border-border-primary last:border-b-0"
                  >
                    <Checkbox
                      ...
                      onChange={(e) => {
                        setSelectedMerchants((prev) => {
                          ...
                          if (e.target.checked) next.add(merchant.id);
                    ...
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      <Image
                        src={merchant.merchantIcon?.imageUpload?.url}
                      ...
                      >
                        {merchant.merchantTranslations[0]?.name || '' } {merchant.merchantName}
                      </Typography>
                    </div>
                    <div className="w-[180px] flex-shrink-0">
                      <SelectField
                        ...
                        value={merchantRoles[merchant.id]}
                        onChange={(val) => {
                          setMerchantRoles((prev) => ({
                            ...prev,
                            [merchant.id]: val as number,
                          }));
                        }}
                        disabled={!selectedMerchants.has(merchant.id)}
                      />
                    </div>
                  </div>
                ))
                  ...
                  onClick={() => {
                    if (allWarnings.length === 0) {
                      createInvite({
                        ...
                        roleId: values["role"],
                        confirmInvite: true,
                        merchantInfo: Array.from(selectedMerchants).map(merchantId => ({
                          merchantId,
                          roleId: merchantRoles[merchantId]
                        })),
                      });
                    } else {
                      showPopup(
