...
import { IOrganizationJuristicTypeMasterDataResponse } from '@/interfaces/organization/organization.response.interface';
import { useUserStore } from '@/store/user.store';

interface StepOrganizationInfoData {
  juristicType?: string;
  idCard?: string;
  organizeName?: string;
  businessType?: string[];
...
interface StepOrganizationInfoProps {
  orgInfoData: StepOrganizationInfoData | null;
  juristicTypeList: Partial<IOrganizationJuristicTypeMasterDataResponse>[];
  onEdit: () => void;
  onAdd: () => void;
  isDisabled: boolean;
}
    ...
    <div className="">
      <div className="p-4 bg-white rounded-t-xl border border-border-primary">
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <Typography variant="paragraph-medium" className="!text-text-primary !font-semibold">
                ...
                <div className="flex flex-col md:flex-row gap-4 md:gap-0">
                  ...
                  <div className="w-full md:w-1/2 flex flex-col gap-1">
                    ...
                    <div className="flex">
                      ...
                      {(isMobile
                        ? (orgInfoData?.businessType?.length || 0) > 1
                        : (orgInfoData?.businessType?.length || 0) > 2) && (
                        <Typography
                          variant="paragraph-small"
              ...
              <>
                <div className="flex flex-col md:flex-row gap-4 md:gap-0">
                  <div className="w-full md:w-1/2 flex flex-col gap-1">
                    ...
                    <Typography variant="paragraph-small" className="!text-text-tertiary">
                      {juristicTypeList?.find((item) => item.id === orgInfoData?.juristicTypeId)
                        ?.label || ''}
                    </Typography>
                  </div>
                  <div className="w-full md:w-1/2 flex flex-col gap-1">
      ...
      >
        <div className="relative">
          ...
          <Divider />
          {orgInfoData?.businessType && orgInfoData?.businessType.length > 0 && (
            <div className="flex flex-col">
              {(orgInfoData?.businessType || []).map((type: string, index: number) => (
                <Typography
