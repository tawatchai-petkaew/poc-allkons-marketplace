...
const TextField: FC<TextFieldProps> = ({
  ...
  const handlePaste = (e: React.ClipboardEvent<HTMLInputElement>) => {
    ...
    if (noSpace && pastedText.includes(" ")) {
      e.preventDefault();
    ...
    if (type === "textOnly") {
      ...
      // Strip non-numeric characters from pasted text for tel type
      const cleanedText = pastedText.replace(/[^0-9]/g, "");
      if (cleanedText !== pastedText) {
        e.preventDefault();
        // Use execCommand to insert cleaned text - this properly triggers React's event system
        document.execCommand("insertText", false, cleanedText);
      }
    }
  };
