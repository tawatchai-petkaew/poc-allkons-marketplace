...
const UploadFileDragger: FC<UploadFileDraggerProps> = ({
  ...
  const { setIsUploadFileCis } = useOrganizationKycStore();
                    ...
                    try {
                      ...
                      const documentIds =
                        file.response.length > 0
                          ...
                          : [file.uid];
                      const res = await deleteFileDraftOrganization(
  ...
  const handleMultipleFileUpload = async (filesToUpload: RcFile[]) => {
    ...
    const uploadPromises = uploadFiles.map(async (fileUpload) => {
      try {
        ...
        const response = await uploadFileDraftOrganization(
          formData,
          Number(draftOrganizeId)
        );
    ...
    <div className={className}>
      ...
      <Dragger
        ...
        className={`custom-allkons-upload-file-dragger ${
          isUploading || isLoading ? 'pointer-events-none opacity-60' : ''
        }`}
      >
