import { getMyPermissionOrganization } from '@/common/api/customer-service/permission';
...
const FormMember: FC<FormMemberProps> = ({
  ...
  const isDisabledRole =
    roleList.find((role: any) => role.id === roleIdValue)?.name === 'OWNER';
  ...
  const roleOptions = filteredRoleList.map((role: any) => ({
  ...
  useEffect(() => {
    setIsHashed({
      firstName: firstNameValue ? true : false,
      lastName: lastNameValue ? true : false,
      middleName: middleNameValue ? true : false,
  ...
  useEffect(() => {
    // Store initial values when form is loaded
    if (!initialValues) {
  ...
  useEffect(() => {
    // Check if form has been changed from initial values
    if (initialValues) {
      const currentValues = form.getFieldsValue();
  ...
  }, [form.getFieldsValue(), initialValues]);
  ...
  return (
    <Form
      ...
      onValuesChange={() => {
        // Trigger re-evaluation of form changes
        if (initialValues) {
          const currentValues = form.getFieldsValue();
          const hasChanged = Object.keys(initialValues).some((key) => {
            const initialValue = initialValues[key as keyof IMemberForm];
