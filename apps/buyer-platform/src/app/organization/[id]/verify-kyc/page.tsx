...
import { juristicTypeList } from '@/common/constants/organization';
import {
...
const OrganizationVerifyDetailPage: FC = () => {
  ...
  const {
    mutate: handleSaveDraftToApprove,
    isPending: isLoadingSaveDraftApprove,
  } = useMutation({
    mutationFn: ({
      payload,
      draftOrganizationId,
  ...
  const { mutate: handleApproveKyc, isPending: isLoadingApproveKyc } =
    useMutation({
      mutationFn: (draftOrganizeId: number) =>
        approveKycOrgnization(draftOrganizeId),
      onSuccess: () => {
        notification.success({
        ...
        router.push(`/organization/${organizeId}`);
      },
      onError: () => {
        showPopup('error', {
          statusCode: 500,
    ...
    setUserForm.setFieldsValue({
      ...
      originalIdCard:
        orgInfoValue?.juristicType === OrganizationTypes.PERSONAL
          ? orgInfoValue?.originalIdCard !== orgInfoValue?.idCard &&
  ...
  useEffect(() => {
    ...
    contactInfoForm.setFieldsValue({
      highestAuthorityName:
        contactInfoValue?.highestAuthority?.highestAuthorityName || '',
      highestAuthorityPosition:
        contactInfoValue?.highestAuthority?.highestAuthorityPosition || '',
      highestAuthorityPhoneNumber:
        contactInfoValue?.highestAuthority?.highestAuthorityPhoneNumber || '',
      highestAuthorityEmail:
        contactInfoValue?.highestAuthority?.highestAuthorityEmail || '',
      ...
      contactShownHighestAuthority:
        contactInfoValue?.contactShownHighestAuthority || false,
    });
  ...
  const handleOrgInfoFinish = (val: any) => {
    const payload = {
      ...
      juristicTypeId:
        juristicTypeList?.find((item: any) => item.id === val.juristicTypeId)
          ?.id || null,
  ...
  const handleContactInfoFinish = (val: any) => {
    const payload = {
      ...
      contact: {
        contactName: val.contactShownHighestAuthority
          ? val.highestAuthorityName
          : val.contactName,
  ...
  const handleAddressInfoSaveDraft = (payload: any) => {
    ...
    handleSaveDraft({
      payload: {
        ...
        orgInfo: JSON.stringify(orgInfoValue) || data?.data.orgInfo || '',
        contactInfo:
          JSON.stringify(contactInfoValue) || data?.data.contactInfo || 'null',
  ...
  const handleFileInfoFinish = async (val: any) => {
    confirmWarning({
      ...
      description: 'ระบบจะทำการส่งข้อมูลเพื่อเข้าสู่กระบวนการอนุมัติ',
      onOk: () => {
        ...
        handleSaveDraftToApprove({
          payload: {
            ...
            orgInfo: JSON.stringify(orgInfoValue) || data?.data.orgInfo || '',
            contactInfo:
              JSON.stringify(contactInfoValue) ||
              data?.data.contactInfo ||
              'null',
            addressInfo:
              JSON.stringify(addressInfoValue) ||
    ...
    handleSaveDraft({
      payload: {
        ...
        contactInfo:
          JSON.stringify(contactInfoValue) || data?.data.contactInfo || 'null',
        addressInfo:
          JSON.stringify(addressInfoValue) || data?.data.addressInfo || 'null',
        taxInfo: data?.data.taxInfo || 'null',
        fileInfo: JSON.stringify(mappedFileData),
        organizeId: Number(organizeId) || 0,
      },
            ...
            <div className="flex justify-between flex-wrap gap-2 items-end">
              <div className="flex flex-col gap-1">
                ...
                >
                  ...
                  คุณสามารถส่งคำขอเพื่ออนุมัติเปิดร้านได้ทันที
                </Typography>
              </div>
            </div>
          </div>
