...
const PhoneWhitelistInfo = () => {
  ...
  const { organizeUuid } = getUserDataFromToken() as any;
  ...
  const { organizeId: organizeProfileId } = getUserDataFromToken() as any;
  const myOrganization = profile?.merchants.find(
    (m) => m.organizeId === organizeProfileId
  )?.organization;
  const { data, isLoading, refetch, isFetching } = useQuery({
    queryKey: ['getAllPhoneWhitelistByOrganization', paramValue],
  ...
  const handleDeletePhoneId = (phoneId: number) => {
    confirmDelete({
      ...
      description: 'หากลบแล้ว สมาชิกจะไม่สามารถใช้งานเบอร์นี้ในระบบได้อีก',
      onOk: () => handleDeletePhone({ organizeUuid, phoneId }),
      okText: 'ลบ',
    });
  };
  ...
  }, [isMobile, isTablet, hashedPhoneRowIds]);
  const handlePageChange = (page: number, pageSize: number) => {
    setParamValue({ page, pageLimit: pageSize });
  };

  useEffect(() => {
    if (phoneList && phoneList.length > 0) {
      setHashedPhoneRowIds((prev: any) => {
        const allUserIds = phoneList.map((user: any) => user.id);
        const newSet = new Set(allUserIds);

        if (
          prev.size === newSet.size &&
          allUserIds.every((id: any) => prev.has(id))
        ) {
          return prev;
        }
      ...
      <div className="flex justify-between px-4 py-4 md:py-0">
        ...
        <Pagination
          ...
          onChange={(page, pageSize) => {
            handlePageChange(page, pageSize || paramValue.pageLimit);
          }}
        />
      </div>
    </div>
