...
import {
  deleteFileDraftOrganization,
  uploadFileDraftOrganization,
...
const UploadFileDragger: FC<UploadFileDraggerProps> = ({
                ...
                <Button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      const res = draftOrganizeId
                        ? await deleteFileDraftOrganization(
                            Number(draftOrganizeId),
                            documentIds
                          )
  ...
  const handleMultipleFileUpload = async (filesToUpload: RcFile[]) => {
    ...
    const uploadPromises = uploadFiles.map(async (fileUpload) => {
      try {
        ...
        const response = draftOrganizeId
  ...
  const uploadProps: UploadProps = {
    ...
    showUploadList,
    fileList: file || [],
    disabled: disabled || isUploading, // Disable upload area when disabled or files are uploading
  ...
  const useAntImgDataTestId = useDataTestIdWithPath(
    usePathname(),
    'ant-image',
    name,
    dataTestId
  );
  const useDraggerDataTestId = useDataTestIdWithPath(
    usePathname(),
    'dragger',
    name,
    dataTestId
