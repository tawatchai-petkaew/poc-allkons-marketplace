...
import { checkTaxId } from '@/common/api/customer-service/organization.api';
import {
  KycOrganizationStatus,
  OrganizationTypes,
} from '@/common/enum/organization.enum';
import { useState, useMemo, useEffect } from 'react';
import { checkIdCard, checkRegisNumber } from '@/common/api/customer-service/user.api';
...
const FormDraftOrgInfo = ({
  ...
  const handleCheckRegistrationNumber = async () => {
    ...
    const originalRegistrationNumber =
      form.getFieldValue('originalRegistrationNumber') || '';
    const isValid = idCardCheck(value);
  ...
  const handleCheckTaxId = async () => {
    ...
    if (taxIdValue === originalJuristic?.taxId) {
      form.setFieldsValue({
        ...
        juristicTypeName: juristicTypeList?.find(
          (juristic: any) => juristic.id === originalJuristic?.juristicTypeId
        )?.value,
        branchName:
    ...
    try {
      ...
      form.setFieldsValue({
        ...
        branchType:
          data.branchName === 'สำนักงานใหญ่' ? 'HEAD_OFFICE' : 'BRANCH',
                                ...
                                <TextField
                                  ...
                                  rules={[
                                    ...
                                    },
                                    {
                                      max: 5,
                                      message:
                                        'ข้อมูลที่กรอกมีความยาวเกินกว่ากำหนด (5 ตัวอักษร)',
                                    },
                                    {
                                      validator: (
                                        _: unknown,
                        ...
                        <Form.Item shouldUpdate noStyle>
                          {({ getFieldValue }) => {
                            const juristicTypeName =
                              getFieldValue('juristicTypeName');
                            if (juristicTypeName === 'OTHER') {
                              return (
                                <TextField
                                  name="remarkTypeOther"
                                  label="ประเภทนิติบุคคลอื่นๆ"
                                  placeholder="กรุณากรอกประเภทนิติบุคคลอื่นๆ"
                                  rules={[
                                    {
                                      required: true,
                                      message: 'กรุณากรอกประเภทนิติบุคคลอื่นๆ',
                                    },
                                    {
                                      max: 50,
                                      message:
                ...
                return (
                  <div className="fixed md:absolute bottom-0 inset-x-0 md:bottom-[-16px] w-full flex justify-end bg-white py-4 pr-4 md:pr-0">
                    <Button
                      ...
                      loading={isLoadingSaveDraft || isLoading}
                    >
                      บันทึกข้อมูล
                    </Button>
                  </div>
