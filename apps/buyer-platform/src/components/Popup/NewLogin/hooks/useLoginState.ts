...
export const useAuthFlow = () => {
  ...
  const [setUserForm] = Form.useForm<FormSetUserFields>();
  ...
  const startCompleteProfileFlow = useCallback(
    ...
    [setFlow, setAccessToken, setStep]
  );
  ...
  const getTransformClass = useCallback(
    (targetStep: Step): string => {
      ...
      if (currentIndex === targetIndex) return 'translate-x-0';
      if (currentIndex < targetIndex) return 'translate-x-full';
