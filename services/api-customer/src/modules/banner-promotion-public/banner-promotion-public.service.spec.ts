...
describe('BannerPromotionPublicService', () => {
  let service: BannerPromotionPublicService;
  let repo: any;
  ...
  beforeEach(async () => {
    ...
    repo = module.get(getRepositoryToken(BannerPromotion));
  ...
  describe('getAll', () => {
    it('should return cached data if available (Cache Hit)', async () => {
      ...
      expect(mockCache.get).toHaveBeenCalledWith('bannerPromotions:1');
