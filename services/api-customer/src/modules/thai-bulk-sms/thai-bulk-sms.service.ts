...
export class ThaiBulkSmsService {
  ...
  constructor(
    ...
    this.baseUrl = this.config.get<string>('THAIBULKSMS_BASE_URL', 'https://api-v2.thaibulksms.com');
    this.apiKey = this.config.get<string>('THAIBULKSMS_API_KEY', '');
    this.apiSecret = this.config.get<string>('THAIBULKSMS_API_SECRET', '');
    this.defaultSender = this.config.get<string>('THAIBULKSMS_SENDER', 'Allkons');
  ...
  private mapProviderError(err: any): { status: number; message: string; details?: string } {
    ...
    if (key.includes('invalid msisdn')) friendly = 'เบอร์โทรศัพท์ไม่ถูกต้อง กรุณาตรวจสอบอีกครั้ง';
    else if (key.includes('quota exceeded')) friendly = 'โควต้าส่ง SMS หมด กรุณาเติมเครดิต';
    else if (key.includes('unauthorized')) friendly = 'ยืนยันตัวตนกับผู้ให้บริการไม่สำเร็จ ตรวจสอบ API Key/Secret';
    else if (key.includes('sender not allowed')) friendly = 'ชื่อผู้ส่ง (Sender) ยังไม่ได้รับอนุมัติหรือไม่ถูกต้อง';
    else if (key.includes('message rejected')) friendly = 'ข้อความถูกปฏิเสธจากผู้ให้บริการ กรุณาปรับข้อความ';
  ...
  async sendSms(dto: SendSmsDto): Promise<ThaiBulkSmsSendResponse> {
    ...
    try {
      ...
      this.logger.error(`ส่งล้มเหลว msisdn=${dto.phoneNumber}`, JSON.stringify(raw?.data ?? raw));
      const mapped = this.mapProviderError(raw);
      throw new HttpException({ message: mapped.message, details: mapped.details }, mapped.status);
    }
  }
}
