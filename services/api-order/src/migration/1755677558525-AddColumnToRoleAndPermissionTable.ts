...
export class AddColumnToRoleAndPermissionTable1755677558525
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "public"."permissions_resource_enum" AS ENUM('ORGANIZATION', 'USER', 'MERCHANT', 'PRODUCT', 'SHOP', 'ORDER', 'MEMBER', 'CREDIT', 'PROMOTION')`,
    );
    await queryRunner.query(
      `ALTER TABLE "permissions" ADD "resource" "public"."permissions_resource_enum"`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."permissions_action_enum" AS ENUM('CREATE', 'READ', 'UPDATE', 'DELETE', 'MANAGE', 'VIEW')`,
    );
    await queryRunner.query(
      `ALTER TABLE "permissions" ADD "action" "public"."permissions_action_enum"`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `COMMENT ON COLUMN "roles"."organizeId" IS 'Organization ID that this role belongs to'`,
    );
  }
