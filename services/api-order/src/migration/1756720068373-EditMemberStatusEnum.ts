...
export class EditMemberStatusEnum1756720068373 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."user_organization_memberstatus_enum" AS ENUM('NONE', 'SENT', 'ACCEPTED', 'DECLINED', 'EXPIRED', 'WAIT_FOR_APPROVE')`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "public"."user_organization_memberstatus_enum_old" AS ENUM('NONE', 'SENT', 'ACCEPTED', 'DECLINED', 'EXPIRED')`,
    );
    await queryRunner.query(
      `ALTER TABLE "user_organization" ALTER COLUMN "memberStatus" DROP DEFAULT`,
    );
