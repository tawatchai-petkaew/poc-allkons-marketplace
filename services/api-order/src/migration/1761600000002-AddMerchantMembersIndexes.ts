...
export class AddMerchantMembersIndexes1761600000002
  implements MigrationInterface
{
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_user_merchants_created_at"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_user_merchants_merchant_user"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_user_merchants_role_id"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_user_merchants_user_id"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_user_merchants_merchant_id"`,
    );
