...
export class AddUserOrgInvitationIndexes1762600000001
  implements MigrationInterface
{
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `DROP INDEX IF EXISTS "idx_invitation_phone_country"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "idx_invitation_organize_status_created"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "idx_user_organization_organize_role"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "idx_user_organization_organize_created"`,
    );
  }
}
