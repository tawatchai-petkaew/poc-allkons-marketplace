...
@Index('uq_merchant_product_variant', ['merchantId', 'productVariantId'], {
  unique: true,
})
export class MerchantProduct extends BaseEntity {
  ...
  @ManyToOne(() => ImageUpload, { nullable: true, onDelete: 'SET NULL' })
  @JoinColumn({ name: 'productCoverImage' })
  ...
  @OneToMany(
    () => MerchantImage,
    (merchantImage) => merchantImage.merchantProduct,
  )
  merchantImages: MerchantImage[];
